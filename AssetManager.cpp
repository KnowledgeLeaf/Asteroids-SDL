...
namespace SDLFramework {
	...
	SDL_Texture* AssetManager::GetTexture(std::string filename, bool managed) {
		...
		if (mTextures[fullPath] == nullptr) {
			mTextures[fullPath] = GraphicsManager::Instance()->LoadTexture(fullPath);
		}
		...
		if (mFonts[key] == nullptr) {
			...
			if (mFonts[key] == nullptr) {
				std::cerr << "Unable to load font " << filename << "! TTF Error: "
		...
		if (mText[key] == nullptr) {
			...
			mText[key] = GraphicsManager::Instance()->CreateTextTexture(font, text, color);
		}
