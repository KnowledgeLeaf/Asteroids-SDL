...
Star::Star(int layer)
	...
	Position(Vector2((float)(mRand->RandomInt() % GraphicsManager::Instance()->SCREEN_WIDTH),(float)(mRand->RandomInt() % GraphicsManager::Instance()->SCREEN_HEIGHT)));
...
void Star::ScrollStar() {
	...
	if (pos.y > GraphicsManager::Instance()->SCREEN_HEIGHT) {
		pos.y = 0.0f;
		pos.x = (float)(mRand->RandomInt() % GraphicsManager::Instance()->SCREEN_WIDTH);
		Position(pos);
	}
}
